...
The RPM spec file provided here is for Scicoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://scicoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Scicoin git repository](https://github.com/scicoin/scicoin) but are not part of
...
Currently only `Source10` is used, for the example `scicoin.conf` file.
...
in the scicoin git repository. At present only an SVG version of the Scicoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the scicoin
...
needed when 0.12.1 is released, a proper fix is already in the Scicoin git
master branch.
...
idea to build Scicoin Core against a static version of that library compiled
...
The `scicoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `scicoin-server` package. At this point, most RPM based
...
The `scicoin-server` package should have SELinux support. How to properly do
...
Scicoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/scicoin/). He has been
packaging Scicoin for Fedora at least since 2012.
...
4. I renamed the `scicoin` package that contains the Qt GUI to `scicoin-core` as
that appears to be how the general population refers to it, in contrast to
`scicoin-xt` or `scicoin-classic`. I wanted to make sure the general population
