...
###   This script attempts to download the signature file SHA256SUMS.asc from scicoin.org
...
WORKINGDIR="/tmp/scicoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
BASEDIR="https://scicoin.org/bin/"
VERSIONPREFIX="scicoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'scicoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Scicoin Core binary release signing key installed?"
   fi
