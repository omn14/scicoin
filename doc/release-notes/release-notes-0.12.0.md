...
Then, to tunnel a SSL connection on 27221 to a RPC server bound on localhost on port 18332 do:

    stunnel -d 27221 -r 127.0.0.1:18332 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bitcoinrpc>
        ProxyPass http://127.0.0.1:7221/
        ProxyPassReverse http://127.0.0.1:7221/
...
- #7209 `f3ad812` test: don't override SCICOIND and SCICOINCLI if they're set (Wladimir J. van der Laan)
...
- #6401 `6db53b4` Add SCICOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
...
- #6504 `b6fee6b` Rationalize currency unit to "SCC" (Ross Nicoll)
